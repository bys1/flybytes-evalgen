...
/**
 *  Generates an interpreter of the given type, for an already generated Flybytes interpreter of the given main type.
 ...
 */
&F genEval(type[&F] funcType, type[&L <: node] mainType, Type evalRet = \void(), Exp(Exp, Symbol) javaToRascal = callVF, list[Stat] preApplyCode = []) {
    Symbol mainSymbol = mainType.symbol;
    Symbol symbol = funcType.symbol.parameters[0];
    if (mainSymbol == symbol) return loadEval(funcType);
...
/**
 *  Returns an interpreter function of the given type for an already generated Flybytes interpreter of the given main type.
 *  Before calling this function, an interpreter should be generated by calling the genEval function with the main function
 *  and actions, AND an interpreter for the given function type should be generated by calling genEval with the given function
 *  type and main type.
 *  If an interpreter is not present, the behaviour of this function is undefined.
 *  The function type should match the function type used when generating the interpreter. This function simply
 *  loads the generated class and creates a Rascal function to call the interpreter.
 ...
 *                  by calling genEval with the function type &X(&L), the type &L should be passed here.
 */
&F loadEval(type[&F] funcType, type[&L <: node] mainType) {
    Symbol mainSymbol = mainType.symbol;
    Symbol symbol = funcType.symbol.parameters[0];
    str mainName = "<mainSymbol.name>Eval";
    str evalName = "<mainName><symbol.name>";
